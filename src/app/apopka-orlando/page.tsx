...
const city = CITIES.find((c) => c.slug === 'apopka-orlando')!;
...
export default function ApopkaOrlandoPage() {
  return (
    <>
      <LocalBusinessLd />
      <ServiceLd name={`${city.name} Moving Services`} slug="apopka-orlando" description={`Professional moving services in ${city.name}, FL. Local, long-distance, residential, commercial, and packing.`} />
      <BreadcrumbsLd items={[
        { name: 'Home', url: SITE.url },
        { name: `${city.name} Movers`, url: `${SITE.url}/apopka-orlando` },
      ]} />
      <CityPage city={city} />
    </>
  );
