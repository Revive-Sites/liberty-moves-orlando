...
const city = CITIES.find((c) => c.slug === 'kissimmee-orlando')!;
...
export default function KissimmeeOrlandoPage() {
  return (
    <>
      <LocalBusinessLd />
      <ServiceLd name={`${city.name} Moving Services`} slug="kissimmee-orlando" description={`Professional moving services in ${city.name}, FL. Local, long-distance, residential, commercial, and packing.`} />
      <BreadcrumbsLd items={[
        { name: 'Home', url: SITE.url },
        { name: `${city.name} Movers`, url: `${SITE.url}/kissimmee-orlando` },
      ]} />
      <CityPage city={city} />
    </>
  );
